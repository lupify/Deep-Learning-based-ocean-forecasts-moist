...
qgm_data_use = "64-64"
qgm_dir = "/media/volume/qgm1/qgm_sim/64_sim_data/"
sim_nums = list(range(300,310))
moist_data_locs = {}

## writing dict with locations of qgm data
for sim_num in sim_nums:
    moist_data_locs[sim_num] = f"{qgm_dir}cold_seed-{sim_num}/output.3d.nc"

save_dir = "/media/volume/qgm1"
output_dir = f"{save_dir}/qgm_models"
data_dir = f"{output_dir}/model_data"
data_loc = f"{data_dir}/sim_qgm_64-64_1.pkl"
models_dir = f"{output_dir}/models/singleSteps_64-64_1/"
...
## preps data for training and testing, saves data, timestamps and information about the data
...
# iterations
train = [301, 302, 303, 304, 305, 306, 307, 308, 309]
test = [300]
...
for step_method, lambda_fft in param_vars:
    ...
    model_name = f"fno2dD_data-{qgm_data_use}_stepMethod-{step_method}_lambda-{lambda_fft_str}_dataPrep-{data_prep}"
    ...
    ## loading previously existing model
        ...
        ## to get shape of input
        d = {t:moists_keep_fno[t] for t in test}
        inputs, targets = datau.load_data_fno(d, data_prep, data_prep_args)
    ...
    # for noise in list(range(0,10)):
        ...
        # if True:
          ...
          continue
            ...
            ## mean spectrum for actual
            actual_spectrum = np.abs(fft.rfft(actual, axis = 2)).mean(axis = (0,1))
...
if 1:
    ...
    actual_dir = f"{models_dir}/actual_moist-{test[0]}"
    ...
    for step in np.arange(0,max_tstep_animation,1):
        ...
        plotting.plot_2d_grid_spectrum(actual,
                                       ...
                                       begframe = moists_keep_fno_timestamps[test[0]][0])
