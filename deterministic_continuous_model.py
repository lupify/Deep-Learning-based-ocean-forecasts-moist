...
path_outputs = '/home/exouser/ocean_reanalysis_5day/outputs'
...
FF=nc.Dataset('/home/exouser/ocean_reanalysis_5day/EnKF_surface_1993to2021_5day_gom.nc')
...
psi_test_input_Tr_torch  = torch.cat((psi_test_input_Tr_torch_norm_level1,psi_test_input_Tr_torch_norm_level2,psi_test_input_Tr_torch_norm_level3),1)
psi_test_label_Tr_torch  = torch.cat((psi_test_label_Tr_torch_norm_level1,psi_test_label_Tr_torch_norm_level2,psi_test_label_Tr_torch_norm_level3),1)
...
psi_train_input_Tr_torch  = torch.cat((psi_train_input_Tr_torch_norm_level1,psi_train_input_Tr_torch_norm_level2,psi_train_input_Tr_torch_norm_level3),1)
psi_train_label_Tr_torch  = torch.cat((psi_train_label_Tr_torch_norm_level1,psi_train_label_Tr_torch_norm_level2,psi_train_label_Tr_torch_norm_level3),1)
...
#        loss = regular_loss(output, label_batch.cuda())
        ...
        loss.backward()
        optimizer.step()
        output_val = directstep (net,psi_test_input_Tr_torch[0:num_samples,:,:,:].reshape([num_samples,3,Nlat,Nlon]))
        val_loss = spectral_loss(output_val, psi_test_label_Tr_torch[0:num_samples,:,:,:].reshape([num_samples,3,Nlat,Nlon]).cuda(),wavenum_init,wavenum_init_ydir,lamda_reg,(torch.tensor(ocean_grid)).cuda())
...
torch.save(net.state_dict(), './BNN_3levels_ocean_spectral_loss_wavenum'+str(wavenum_init)+'lead'+str(lead)+'.pt')
...
M=20
autoreg_pred = np.zeros([M,3,np.size(psi_test_label_Tr,2),np.size(psi_test_label_Tr,3)])

for k in range(0,M):

  if (k==0):

    out = (directstep(net,psi_test_input_Tr_torch[k,:,:,:].reshape([1,3,Nlat,Nlon]).cuda()))
    autoreg_pred[k,:,:,:] = out.detach().cpu().numpy()

  else:

    out = (directstep(net,torch.from_numpy(autoreg_pred[k-1,:,:,:].reshape([1,3,Nlat,Nlon])).float().cuda()))
    autoreg_pred[k,:,:,:] = out.detach().cpu().numpy()
...
autoreg_pred = np.concatenate((autoreg_pred_level1_denorm,autoreg_pred_level2_denorm,autoreg_pred_level3_denorm),axis=1)


matfiledata = {}
matfiledata[u'prediction'] = autoreg_pred
matfiledata[u'Truth'] = psi_test_label_Tr
hdf5storage.write(matfiledata, '.', path_outputs+'predicted_3level_ocean_spectral_loss_5day_train_wavenumber'+str(wavenum_init)+'directstep_lead'+str(lead)+'.mat', matlab_compatible=True)
